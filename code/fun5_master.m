...
global m bdry p p1 p2   x0 L ml mr Av Bv vspur VF1 VF2 VV1 VV2 line A5
...
for i = 1:m
    ...
    if((i<=ml) || (i>L && i<=L+mr))     %Falls i ein Fussgänger ist...
        ...
        for j=1:m
            ...
            %Ellpisengleichung im Parameter phi mit M(x0,y0), alpha ist die Verdrehung
            ...
            X0=x(i,1);
            Y0=x(i,2);
            b= 2 ;    %Die kurze halbachse
            a=(2*b)+(15-(2*b))*norm([u(2*i-1),u(2*i)])/vmax2; %Die lange Halbachse muss eine Fkt. der aktuellen Geschwindigkeitn sein. Sie soll
           %proportional mit der Geschwindikgeit zunehmen (weiter voraus
            %schauen bei grösserer Geschwindigkeit). amax=15, amin=2*b
            
            %Die Koordinaten des Ellipsenrandpunktes
            x1=X0+a*cosphi*cosalpha - b*sinphi*sinalpha;
            y1=Y0+a*cosphi*sinalpha + b*sinphi*cosalpha;
            ...
            if((i~=j)&&(cosphi>0)&&(L2<L1));    %cosphi>0 um nur die vordere hälfte der Ellypse zu berücksichtigen und L2 muss innerhalb der Ellipse L1 sein
                rab = r(i) + r(j);%summe der beiden radien der Personen
                vab = [u(2*j-1),u(2*j)] *(-eij)' + [u(2*i-1),u(2*i)] *eij'; %geschwindigkeiten auf die direkte verbindung von i,j projiziert
                
                h=A3a;
                if vab < 0
                    h = 0;    %Falls die j-te Person von der i-ten entfernt, soll keine Sichtkraft wirken (gleichbedeutend mit vab = 0)
                end
                
                fab = fab + h*exp((rab - L2)/B3a)*-eij; %Aufsummierung der Kräfte aller anderen Personen (A1a, Proportionalitätskonstante; (rab-dab) wenn dab klein ist wird dieser ausdruck exponentiell grösser!
              
                
            if (L2 < VS)
                fab= fab +((A4a*exp((rab - L2)/B4a))*-eij); %eine weitere Kraft, welche die physikalische komponente simulieren soll. sie ist unabhängig von lambda, also dem Sichtbereich
                %Parameter: Ai wachsend verstärken die Kraft, Bi wachsend
                %schwächt die Kraft
            end
            end
    ...
    elseif uv(i) ==1  % falls vorher (t-1) ein unfall war im nächsten zeitschritt soll es möglich sein ein Unfall zu haben falls in diesem Zeitschritt kein unfall ist
        ...
        if j ~= i && dij < 0.4; % wenn ein unfall ist im nächsten zeitschritt wieder nicht möglich einen Unfall zu bauen
            uv(i) = 1;
    ...
    if line == 1
        ...
        for j = 1:length(vspur);
            if ((norm(x(i,:) - vspur(j,:))) < daV) %wenn der j-te Randpunkt näher liegt als der bisherig als nächsten betrachteten
        ...
        if (i<=ml) || (i>L&&i<=L+mr) %falls ein Fussgänger r
            if x(i,2)>9; %ausserhalb der Velospur
                faVS = VF1*exp(vsd/VF2)*naV;    %vsd ist eine variable, somit faVS ist variabel
           
            else  % falls ein fussgänger und innerhalb der Velospur
            	faVS = VF1*exp((r(i)/VF2))*-naV;    %r(i) ist konstant, somit faVS ist auch konstant
            end
        end
        ...
        if (i>ml && i<=L) || (i>L+mr)  % falls ein Velofahrer r
            if x(i,2) <= 9; %und in der Velospu
                faVS = (VV1*(exp(vsd/VV2)))*naV;
           
            else % falls ein Velofahrer und nicht in der Velospur
                faVS = (VV1*(exp(r(i)/VV2)))*-naV;
            end
        end
    ...
    if i>ml+1 && i<=L    %nur die Velofahrer der linken Seite
        ...
        for j=ml+1:L
            ...
            if (i ~= j && (x(j,1)-x(i,1))>0)  %Der j-te Velofahrer muss vor dem i-ten sein, damit der i-te sich hinter ihn gesellen kann
                ...
                if(dij<Dij)
                    Dij=dij; %Dij übernimmt den kleineren Abstand zum j-ten Velofahrer
        ...
        if(k~=0)  %wenn ein j-ter Velofahrer vor dem i-ten Velofraher fährt
            if(norm(u(2*k-1),u(2*k))>0)    %Wenn die geschwindigkeit des vorderen nicht null ist
                ...
                if (dij>3*r(k))     %die senkrechtkraft soll nur wirken, wenn der abstand zum vorderen grösser als dreimal der Eigenradius ist!
    ...
    elseif  i>L+mr     % nur die Velofahrer der rechten Seite
        fv = [0,0]; % Die Anziehungskraft, welche auf den i-ten Velofahrer wirkt
